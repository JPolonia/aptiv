...
dict_FinalDataFile, dict_OptionalFinalDataFile, dict_OptionalFeatures = final_dictionary(dictFile,revAssembliesList, positions, verticalAssemblyDict)

tstart = datetime.now()
print(tstart)

import json
f = open("files/VerifiedPartnunmbers.json", "w+")
f.write(json.dumps(dict_FinalDataFile, indent=4, sort_keys=True))
f = open("files/VerifiedOptionalPartnunmbers.json", "w+")
